...
/** Gộp pin từ id_product accounts_admin (CSV), ccp_product_ids, options.assignProductId. */
function mergePinnedProductCandidates(options = {}) {
  const chunks = [];
  const pushCsv = (v) => {
    if (v == null) return;
  ...
  pushCsv(options.idProductFromAccount);
  if (options.assignProductId) {
    chunks.push(String(options.assignProductId).trim());
  }
  if (Array.isArray(options.assignProductIds)) {
    for (const x of options.assignProductIds) {
      if (x != null && String(x).trim() !== "") chunks.push(String(x).trim());
    }
  }
  if (Array.isArray(options.pinnedCcpProductIds)) {
    chunks.push(...options.pinnedCcpProductIds);
  }
  return normalizeCcpProductIdList(chunks);
}

function normalizedRowProductKey(entry) {
  const extracted = extractProductId(entry);
  const arr = normalizeCcpProductIdList(extracted);
  return arr[0] || "";
}

/**
 * Chọn một hàng JIL products để PATCH add user — tránh ô đầu list (hay là Free Members).
 */
function pickProductRowForAssign(orgProductsList, pinnedNormIds = []) {
  const list = Array.isArray(orgProductsList) ? orgProductsList : [];
  if (list.length === 0) {
    throw new Error("product_id_missing");
  }

  const isExpiredRow = (p) => String(p?.status || "").toLowerCase() === "expired";

  /** @param {(p: unknown) => boolean} pred */
  const firstMatching = (pred) => list.find(pred) || null;

  for (const pin of pinnedNormIds) {
    if (!pin) continue;
    const hit =
      firstMatching((p) => !isExpiredRow(p) && normalizedRowProductKey(p) === pin) ||
      firstMatching((p) => normalizedRowProductKey(p) === pin);
    if (hit) {
      logger.info("[adobe-v2] resolveAssignableProductId: dùng pin khớp JIL (%s)", pin);
      return hit;
    }
  }

  const ccpKeys = normalizeCcpProductIdList(
    extractCcpSeatProductIdsFromOrgProductsList(list)
  );
  for (const k of ccpKeys) {
    const hit =
      firstMatching((p) => !isExpiredRow(p) && normalizedRowProductKey(p) === k) ||
      firstMatching((p) => normalizedRowProductKey(p) === k);
    if (hit) {
      logger.info("[adobe-v2] resolveAssignableProductId: dùng seat CCP từ JIL list (%s)", k);
      return hit;
    }
  }

  const ccpPrefer = firstMatching(
    (p) => !isExpiredRow(p) && isCcpLikeProduct(p) && !isNonProMembershipProduct(p)
  );
  if (ccpPrefer) {
    logger.info(
      "[adobe-v2] resolveAssignableProductId: dòng CCP-like không Member/Free trong list"
    );
    return ccpPrefer;
  }

  const skipMemberPrefer = firstMatching(
    (p) => !isExpiredRow(p) && !isNonProMembershipProduct(p)
  );
  if (skipMemberPrefer) {
    logger.info(
      "[adobe-v2] resolveAssignableProductId: bỏ gói Member/Free trong list → chọn ô khác không expired"
    );
    return skipMemberPrefer;
  }

  const anyNonExpired =
    list.find((p) => !isExpiredRow(p)) || list[0];
  logger.warn("[adobe-v2] resolveAssignableProductId: fallback không lọc được CCP, dùng ô đầu không expired/list[0]");
  return anyNonExpired || list[0];
}
...
async function resolveAssignableProductId(page, orgToken, headers, options = {}) {
  ...
  const pins = mergePinnedProductCandidates(options);
  const picked = pickProductRowForAssign(list, pins);
  const productId = String(extractProductId(picked) || "").trim();
...
async function runAddUsersFlow(page, userEmails = [], options = {}) {
  ...
  if (!assignOptions.idProductFromAccount && options.id_product) {
    assignOptions.idProductFromAccount = options.id_product;
  }
  const productId = await resolveAssignableProductId(page, orgToken, headers, assignOptions);
