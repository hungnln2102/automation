...
/** Chuẩn hóa adobe_org_id từ DB: hex hoặc hex@AdobeOrg → hex. */
function normalizeSeedOrgHex(raw) {
  const s = String(raw || "").trim();
  if (!s) return null;
  const m = s.match(/^([A-Fa-f0-9]{20,})(?:@AdobeOrg)?$/i);
  return m ? m[1] : null;
}
...
async function runCheckProductFlow(page, flowOpts = {}) {
  return withRecoverableRetry(
    "B12-check-product",
    async () => {
      const seedHex = normalizeSeedOrgHex(flowOpts.seedOrgId ?? flowOpts.existingAdobeOrgId);
      const productsEntryUrl = seedHex
        ? `https://adminconsole.adobe.com/${seedHex}@AdobeOrg/products`
        : ADMIN_PRODUCTS;
      logger.info(
        "[adobe-v2] B12: adminconsole/products (url=%s%s)",
        productsEntryUrl.slice(0, 80),
        seedHex ? ", seedOrg=DB" : ""
      );
      await page.goto(productsEntryUrl, { waitUntil: "domcontentloaded", timeout: 35000 }).catch(() => {});
      ...
      const orgId = extractOrgIdFromUrl(productsUrl) || seedHex || null;
