...
/** Khớp URL JIL users (bps-il) dù Adobe encode @ thành %40 hoặc path khác đợi. */
function orgTokenMatchesJilOrganizationsPath(url, token) {
  const u = String(url || "");
  const t = normalizeOrgToken(token);
  if (!t || !u.includes("/jil-api/v2/organizations/")) return false;
  if (!u.includes("/users")) return false;
  const encFull = encodeURIComponent(t);
  const encAt = t.replace(/@/g, "%40");
  const bare = t.replace(/@AdobeOrg$/i, "");
  const candidates = [
    t,
    encFull,
    encAt,
  ...
  for (const seg of candidates) {
    if (!seg) continue;
    if (u.includes(`/organizations/${seg}/`)) return true;
  }
  try {
    const lower = u.toLowerCase();
    if (bare && lower.includes(`organizations/${bare.toLowerCase()}%40adobeorg`)) {
      return true;
    }
  } catch (_) {}
  return false;
...
async function captureUsersApiHeaders(page, orgToken) {
  ...
  const reqPromise = page.waitForRequest(
    (req) => {
      ...
      return (
        req.method() === "GET" &&
        orgTokenMatchesJilOrganizationsPath(url, token) &&
...
module.exports = {
  ...
  normalizeOrgToken,
  orgTokenMatchesJilOrganizationsPath,
