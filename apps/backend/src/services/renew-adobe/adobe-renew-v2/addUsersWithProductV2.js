 ...
 * Luồng mặc định: B1–B9 (onlyLogin) → /users → create user (abpapi) + assign product (PATCH) → snapshot.
 * options.skipLogin: bỏ runCheckFlow; cần persistent profile đã đăng nhập (không fallback ephemeral).
...
async function addUsersWithProductV2(adminEmail, password, userEmails, options = {}) {
  ...
  try {
    ...
    if (skipLogin) {
      logger.warn(
        "[adobe-v2] addUsersWithProductV2: skipLogin nhưng không mở được persistent profile: %s",
        profileErr.message
      );
      return {
        success: false,
        error: `skipLogin yêu cầu persistent profile Playwright (${profileErr.message})`,
        savedCookies: null,
        snapshot: null,
      };
    }
  ...
  try {
    if (skipLogin) {
      logger.info(
        "[adobe-v2] addUsersWithProductV2: skipLogin — không chạy runCheckFlow, add=%d email",
        emails.length
      );
    } else {
    ...
    const addResult = await runAddUsersFlow(page, emails, {
      ...
      idProductFromAccount:
        options.idProductFromAccount != null ? String(options.idProductFromAccount).trim() : "",
