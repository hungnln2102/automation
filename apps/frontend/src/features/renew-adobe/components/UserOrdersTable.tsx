/**
 * Bảng hiển thị Tên Khách Hàng, Email, Profile, Tình trạng Gói, Hạn Sử Dụng, Thao Tác.
 *
 * Luồng dữ liệu:
 * API trả về dữ liệu từ system_automation.list_user và accounts_admin.
 */
...
import { AddListUserModal } from "@/features/renew-adobe/components/AddListUserModal";
  ...
  return (
    <div className="rounded-[18px] bg-gradient-to-br from-slate-800/65 via-slate-700/55 to-slate-900/65 border border-white/15 p-4 lg:p-6 shadow-[0_20px_55px_-30px_rgba(0,0,0,0.7)] backdrop-blur-sm">
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-1 min-w-0">
          ...
          <p className="text-xs text-white/50">
            Tên Khách Hàng, Email, Profile, Tình trạng Gói, Hạn Sử Dụng
          </p>
        </div>
        <div className="flex shrink-0 flex-col gap-2 sm:flex-row sm:items-center sm:justify-end">
          {onFixAllUsers && fixableEmailsInView.length > 0 ? (
            <button
              ...
              disabled={!!fixingId || !!deletingId || !!fixAllProgress}
              className="rounded-xl bg-amber-500/20 text-amber-300 border border-amber-400/40 px-4 py-2 text-sm font-semibold hover:bg-amber-500/30 disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap transition-colors"
            >
              {fixAllProgress
      ...
      <AddListUserModal
        ...
        onCreated={() => setListRefreshTick((t) => t + 1)}
      />
